...
def page(events,bjzs,ext,errors,now,notes,goalodds,movements,forced,confidence_rows,api_rows):
 ...
 for e in events:
  ...
  move=movements.get(e['code'],'首次快照，等待下一次对比')
  benchmark += f"<br><b>变化：</b>{escape(move)}"
...
def main():
 ...
 for e in events:
  ...
  f=c['flags']; conf_html+=f"<tr><td>{escape(e['code'])}</td><td>{f['china']}</td><td>{f['average']}</td><td>{f['external']}</td><td>{f['fundamental']}</td><td>{f['injury']}</td><td>{f['movement']}</td><td>{f['model']}</td><td><b>完整度 {c['score']}/100 · {c['level']}</b><br><span class='small'>中{c['breakdown']['中国竞彩']} 百{c['breakdown']['百家平均']} 模{c['breakdown']['进球模型']} API{c['breakdown']['API赛程']} AI{c['breakdown']['AI搜索']+c['breakdown']['AI事实']} 外{c['breakdown']['外部机构']} 首{c['breakdown']['首发确认']}</span></td><td>{c['kelly']*100:.2f}%</td><td>{c['forced_cap']*100:.2f}%上限</td></tr>"
