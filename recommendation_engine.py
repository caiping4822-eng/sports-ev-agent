"""Transparent, conservative recommendation rules shared by dashboard and lock ledger."""
...
def factual_absences(event,research):
 """Count only source-backed confirmed adverse facts, never predictions or 'motivation'."""
...
def build_decisions(events,bjzs):
 """Return every China-lottery event with explicit market/fact/risk components.
    Facts can only make small capped directional adjustments; missing facts never invent one.
    """
 ...
 for event in events:
  ...
  ar=aby.get(event['code'],{}); research=ar.get('research',{}) if isinstance(ar,dict) else {}
  ap=api.get(event['code'],{}); fp=fby.get(event['code'],{}); er=external.get(event.get('source_match_id'),{})
  books=len(er.get('books',[])) if er.get('available') else 0
  # Data-confidence score: availability only, not an assertion of predictive strength.
  score=35
  ...
  # Transparent rule: each verified adverse fact moves H/A by only 0.4pp, cap 2pp.
  direction=max(-.02,min(.02,(away_bad-home_bad)*.004))
  risk_penalty=min(.012,.002*len(risks))
  base=[max(0,p-.02) for p in market_p]
  final=[max(0,base[0]+direction-risk_penalty),max(0,base[1]-risk_penalty),max(0,base[2]-direction-risk_penalty)]
  ...
  # A global pick values probability and evidence quality, and cannot ignore conflict risk.
  ...
  output.append({'code':event['code'],'match':event['home']+' vs '+event['away'],'odds':odds,
   'market_p':market_p,'base_p':base,'conservative_p':final,'ev':ev,'forced_i':forced_i,'strict_i':strict_i,'strict':strict,
   'confidence':score,'conf_label':confidence,'global_score':global_score,'home_adverse':home_bad,'away_adverse':away_bad,
   'direction_adjustment':direction,'risk_penalty':risk_penalty,'confirmed':research.get('confirmed',[]) or [],
   'uncertain':research.get('uncertain',[]) or [],'risks':risks,'context':research.get('competition_context',[]) or [],
   'motivation':research.get('motivation_evidence',[]) or [],'form_schedule':research.get('form_schedule',[]) or [],'gaps':gaps})
 return output
...
def forced_payload(decision):
 i=decision['forced_i']
 return {'selection':LABELS[i],'odds':decision['odds'][i],'probability':decision['conservative_p'][i],
  'market_probability':decision['market_p'][i],'base_probability':decision['base_p'][i],
  'ev':decision['ev'][i],'stake_units':1,'type':'per_match','probability_source':'百家平均去水＋确认事实小幅调整＋来源风险惩罚',
  'home_adverse_facts':decision['home_adverse'],'away_adverse_facts':decision['away_adverse'],
  'direction_adjustment':decision['direction_adjustment'],'risk_penalty':decision['risk_penalty'],
  'data_confidence':decision['confidence'],'global_composite_score':decision['global_score']}
